 ...
 * These checks are performed on frontend (for UX) and backend (for security).
 */
...
const BLOCKED_PATTERNS: Array<{ pattern: RegExp; message: string; category: string }> = [
  ...
  // Process killing (allow specific monitoring-safe patterns separately)
...
// Warning patterns - allowed but flagged for review
const WARNING_PATTERNS: Array<{ pattern: RegExp; message: string }> = [
  { pattern: /\bsudo\b/, message: 'Script uses sudo - ensure the agent user has proper sudoers configuration' },
  { pattern: /\bsu\s+-/, message: 'Script switches user with su - verify this is intended' },
  { pattern: /\beval\b/, message: 'Script uses eval - verify the evaluated content is safe' },
  { pattern: /\$\([^)]+\)/, message: 'Script uses command substitution - verify the commands are safe' },
  { pattern: /`[^`]+`/, message: 'Script uses backticks for command substitution - verify the commands are safe' },
  { pattern: /\bwget\b/, message: 'Script downloads files - verify URLs are trusted' },
  { pattern: /\bcurl\b/, message: 'Script makes HTTP requests - verify URLs are trusted' },
  { pattern: />\s+[^\/]/, message: 'Script writes to files - verify paths are intended' },
  { pattern: /\bchmod\b/, message: 'Script changes file permissions - verify this is intended' },
  { pattern: /\bchown\b/, message: 'Script changes file ownership - verify this is intended' },
];
...
export function validateScript(script: string): ScriptValidationResult {
  ...
  // Check for warning patterns (only if no errors)
  if (errors.length === 0) {
...
/**
 * Get a list of all blocked command categories
 */
export function getBlockedCategories(): string[] {
  const categories = new Set(BLOCKED_PATTERNS.map(p => p.category));
  return Array.from(categories);
}

/**
 * Check if a specific command is blocked
 */
